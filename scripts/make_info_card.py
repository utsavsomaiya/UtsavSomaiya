...
ROWS = [
    ("host",),
    ("kv", "Now", "Product & Tech Consultant @ H2H"),
    ("kv", "Work", "Building YouNegotiate"),
    ("kv", "Based", "Rajkot, India"),
    ("kv", "Focus", "Calm products & practical AI"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Backend", "Laravel, PHP, FastAPI, PostgreSQL"),
    ("kv", "Frontend", "Livewire, Inertia, React, Tailwind"),
    ("kv", "AI", "OpenAI, pgvector, retrieval workflows"),
    ("kv", "Cloud", "AWS, Docker, GitHub Actions, Vercel"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Building trust-first product systems"),
    ("bul", "Laravel Ahmedabad speaker & contributor"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">utsav@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">utsav</tspan><tspan fill="{MUTED}">@</tspan>'
